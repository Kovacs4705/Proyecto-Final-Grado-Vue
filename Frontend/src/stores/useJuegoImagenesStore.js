  ...
  actions: {
    ...
    async fetchImagenes() {
      ...
      try {
        ...
        this.error = err.message || 'Error desconocido al cargar imágenes'
      } finally {
    ...
    async fetchImagenPorId(id) {
      ...
      try {
        const res = await fetch(`http://localhost:8000/api/juego-imagenes/${id}`)
        if (res.status === 404) {
        ...
        this.error = err.message || 'Error desconocido al obtener la imagen'
      } finally {
    ...
    async actualizarImagen(id, payload) {
      ...
      try {
        ...
        this.error = err.message || 'Error desconocido al actualizar imagen'
      } finally {
        this.isSubmitting = false
      }
    },
    ...
    async eliminarImagen(id) {
      ...
      try {
        ...
        this.error = err.message || 'Error desconocido al eliminar imagen'
      } finally {
