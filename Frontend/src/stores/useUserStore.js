  ...
  actions: {
    ...
    async fetchUsers({ pagina = 1, registrosPorPagina = 10 } = {}) {
      ...
      try {
        ...
        this.error = err.message || 'Error desconocido al cargar usuarios'
        this.users = []
      } finally {
    ...
    async fetchUserByDni(dni) {
      ...
      try {
        const res = await fetch(`${API_URL}/usuarios/${encodeURIComponent(dni)}`)
        if (res.status === 404) throw new Error('Usuario no encontrado')
        if (!res.ok) throw new Error(`Error ${res.status} al obtener usuario`)
        this.userActual = await res.json()
      } catch (err) {
        this.error = err.message || 'Error desconocido al obtener usuario'
    ...
    async createUser(payload) {
      ...
      try {
        ...
        if (!res.ok) throw new Error(`Error ${res.status} al crear usuario`)
        return await res.json()
      } catch (err) {
        this.error = err.message || 'Error desconocido al crear usuario'
        return null
      } finally {
    ...
    async updateUserByDni(dni, payload) {
      ...
      try {
        ...
        if (res.status === 404) throw new Error('Usuario no encontrado')
        if (!res.ok) throw new Error(`Error ${res.status} al actualizar usuario`)
        return await res.json()
      } catch (err) {
        this.error = err.message || 'Error desconocido al actualizar usuario'
        return null
      } finally {
        this.isSubmitting = false
      }
    ...
    async deleteUserByDni(dni) {
      ...
      try {
        ...
        if (res.status === 404) throw new Error('Usuario no encontrado')
        if (!res.ok) throw new Error(`Error ${res.status} al eliminar usuario`)
        return await res.json() // p.ej. { "message": "Usuario eliminado correctamente" }
      } catch (err) {
        this.error = err.message || 'Error desconocido al eliminar usuario'
        return null
      } finally {
    ...
    async searchUsers(parametro) {
      ...
      try {
        const res = await fetch(
          `${API_URL}/usuarios/buscarPorParametro?parametro=${encodeURIComponent(parametro)}`
        )
        if (!res.ok) throw new Error(`Error ${res.status} al buscar usuarios`)
        this.users = await res.json()
      } catch (err) {
        this.error = err.message || 'Error desconocido al buscar usuarios'
        this.users = []
      } finally {
        this.isLoadingList = false
      }
